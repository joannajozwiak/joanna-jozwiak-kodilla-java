...
public class BoardTestSuite {
    ...
    void testAddTaskListAverageWorkingOnTask(){
        ...
        //When
        List<TaskList> inProgressTasksNow = new ArrayList<>();
        inProgressTasksNow.add(new TaskList("In progress"));
        long longTasks = project.getTaskLists().stream()
                .filter(inProgressTasksNow::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .map(Task::getCreated)
                .map(d -> Duration.between(d.atStartOfDay(),LocalDate.now().atStartOfDay()).toDays())
        ...
        long average = longTasks / numberOfTasks;


        //Then
        assertEquals(10, average);                                       // [9]
    }
    }
